...
Associative Operations and Invertible Operations required

1. build: O(m)
2. query: O(log m)
3. update: O(log m)

Fenwick Tree = Binary jumps + partial prefix sums
i += LSOne(i) // to go to the parent node
i -= LSOne(i) to go to the ancestor node
*/
...
public:
    ...
    FenwickTree(int m, const vi &s){ // create FT based on s
        vll f(m + 1, 0);
        int n = (int)s.size();
        for (int i = 0; i < n; ++i) // do the conversion first
            ++f[s[i]];                          // in O(n)
        build(f);                               // in O(m)
    }
    ...
    int select(ll k){ // O(log m)
        int p = 1;
        int m = (int)ft.size();
        while (p * 2 < m) p *= 2; // find greatest power of 2 <= m
        int i = 0;
        while (p){
            if (k > ft[i + p]){
                k -= ft[i + p];
...
int example(){
    FAST_IO;
    // demo of Fenwick Tree and its variants
